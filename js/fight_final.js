...
orda.push(YOU);
massfight(orda, 15, 11);
if (r <= 15 && deadEnemy >= oppositionArray.length) {
    document.write('<p>Вы честно выдержали это испытание, сумев сохранить жизнь в неравном бою!<br> После всего, осмотрев пещеру, вы находите в дальнем отнорке огромный сундук, доверху наполненный монетами, украшениями и посудой из драгоценных металлов.</p><p>Возвращаясь с добычей в деревню вы по какому-то наитию отсыпаете часть монет старцу, который будто и не сходил с места за это время.<br>В ответ вас снова будто просвечивают насквозь взглядом необычно ясных глаз...</p>');
}
else {
