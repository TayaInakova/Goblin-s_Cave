...
if (r<=10 && dp>=np.length){
    ...
    document.write('<p>Противники готовы порезать вас на ленточки,<br> но вам уже почти нечего им противопоставить - силы на исходе.<br> Вы уже готовы были встретиться с богами, как вдруг услышали крик на незнакомом языке, непохожем на гоблинское наречие.<br> В гущу боя ворвался разумный, расовую принадлежность которого было несложно определить даже за то мгновение, что вы смотрели друг на друга</p>');
let Elf= new Character(200,200,'Гилдерой',15,8,20,'*',3,5,4,100);
Elf.character_list();
orda.push(Elf);
massfight(orda,10,20);
};
if (r<=10 && dp>=np.length && ds==0){
